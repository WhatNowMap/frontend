  ...
  const handleCategoryChange = (e: ChangeEvent) => {
    ...
    for (const item of checkbox.form?.elements!) {
      if (
        ...
        (item as HTMLInputElement).name != checkbox.name
      ) {
        (item as HTMLInputElement).checked = false;
      }
    }

    if (checkbox.checked) category = checkbox.name;
    else category = "all";
    ...
    navigate(
      "/list/" +
        (defaults.Categories.includes(
          ...
          : "all") +
        "/" +
        (defaults.SortingCriteria.includes(
          sort as "Recommended" | "Distance" | "Freshness" | "Attendees"
        )
          ? sort.toLowerCase()
          : "recommended") +
        "/" +
